################ Beginning Student Language (BSL) Interpreter in Python
...
def add_globals(self):
    ...
    self.update({
        ...
        'port?':
            lambda x: isa(x, file),
        'apply':
            lambda proc, l: proc(*l),
        'eval':
            lambda x: eval(expand(x)),
        'load':
            lambda fn: load(fn),
        'call/cc':
            callcc,
        'open-input-file':
            open,
        'close-input-port':
            lambda p: p.file.close(),
        'open-output-file':
            lambda f: open(f, 'w'),
        'close-output-port':
            lambda p: p.close(),
        'eof-object?':
            lambda x: x is eof_object,
        'read-char':
            readchar,
        'read':
            read,
        ...
        'display':
            lambda x, port=sys.stdout: port.write(x if isa(x, str) else
                                                  to_string(x))
    })
    return self
...
def expand(x, toplevel=False):
    ...
    if not isa(x, list):  # constant => unchanged
        ...
        vars, body = x[1], x[2:]
        require(x, (isa(vars, list) and all(isa(v, Symbol) for v in vars)) or
...
def let(*args):
    ...
    require(
        x,
        all(
            isa(b, list) and len(b) == 2 and isa(b[0], Symbol)
            for b in bindings), "illegal binding list")
    vars, vals = zip(*bindings)
    return [[_lambda, list(vars)] + list(map(expand, body))] + list(
        map(expand, vals))
